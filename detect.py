...
Usage:
    python detect.py --source data/images/crabs.png --weights yolov5s.pt --img 640 # runs yolo on image
    python detect.py --source data/images/stream.png --weights best.pt --img 640 --use-screen # takes screenshot of monitor and runs yolo
...
def GRABMSS_screen():
    ...
    im.save('data/images/stream.png')
...
def run(weights='yolov5s.pt',  # model.pt path(s)
        ...
        iou_thres=0.46,  # NMS IOU threshold
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        if save_crop:
                            ...
                            cv2.imwrite(str(increment_path(save_dir / 'crops' / names[c] / f'{p.stem}_full_{frame}.jpg', mkdir=True).with_suffix('.jpg')), imc)
            ...
            # Stream results
            if view_img:
                #cv2.namedWindow(str(p))        # Create a named window
                #cv2.moveWindow(str(p), 40,30)
